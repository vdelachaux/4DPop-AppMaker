		...
		Case of 
			...
			: (Is macOS:C1572)
				...
				If ($path[[1]]=":")
					...
					$folder:=Folder:C1567(Get 4D folder:C485(Database folder:K5:14; *); fk platform path:K87:2).folder(Replace string:C233(Delete string:C232($path; 1; 1); Folder separator:K24:12; "/"))
...
Function build()->$status : Object
	...
	var $BuildApplicationLog; $CodeDesc; $dom; $log; $logPath; $Message : Text
	var $MessageType; $Target; $UUID; $xml : Text
	var $CodeId; $i : Integer
	var $file; $settingsFile : 4D:C1709.File
	...
	$file:=Folder:C1567(fk resources folder:K87:11).file("BuildApp.4DSettings")
	...
	If ($file.exists)
		...
		$status.success:=Bool:C1537(OK)
		
		// This is for the default project
		$file:=Folder:C1567(fk logs folder:K87:17; *).file($UUID+".log.xml")
		...
		If ($file.exists)
			...
			If (Bool:C1537(OK))
				...
				$errors{0}:=DOM Find XML element:C864($BuildApplicationLog; "Log"; $errors)
				
				For ($i; 1; Size of array:C274($errors); 1)
					
					DOM GET XML ELEMENT VALUE:C731(DOM Find XML element:C864($errors{$i}; "MessageType"); $MessageType)
					DOM GET XML ELEMENT VALUE:C731(DOM Find XML element:C864($errors{$i}; "Target"); $Target)
					DOM GET XML ELEMENT VALUE:C731(DOM Find XML element:C864($errors{$i}; "CodeDesc"); $CodeDesc)
					DOM GET XML ELEMENT VALUE:C731(DOM Find XML element:C864($errors{$i}; "CodeId"); $CodeId)
					DOM GET XML ELEMENT VALUE:C731(DOM Find XML element:C864($errors{$i}; "Message"); $Message)
...
Function _getSettings($settingsFile : 4D:C1709.File)->$BuildApp : Object
	...
	var $AdHocSign; $BuildApplicationLight; $BuildApplicationName; $BuildApplicationSerialized; $BuildCompiled; $BuildComponent : Text
	var $BuildCSUpgradeable; $BuildMacDestFolder; $BuildServerApplication; $BuildV13ClientUpgrades; $BuildWinDestFolder; $child : Text
	var $ClientMacFolderToMac; $ClientMacFolderToWin; $ClientMacIconForMacPath; $ClientMacIconForWinPath; $ClientMacIncludeIt; $ClientWinFolderToMac : Text
	var $ClientWinFolderToWin; $ClientWinIconForMacPath; $ClientWinIconForWinPath; $ClientWinIncludeIt; $CurrentVers; $dom : Text
	var $HardLink; $IncludeAssociatedFolders; $IPAddress; $IsOEM; $Item; $itemName : Text
	var $ItemsCount; $LastDataPathLookup; $MacCertificate; $MacCompiledDatabaseToWin; $MacCompiledDatabaseToWinInclude; $MacSignature : Text
	var $name; $PackProject; $parent; $path; $PortNumber; $RangeVersMax : Text
	var $RangeVersMin; $RuntimeVLIncludeIt; $RuntimeVLMacFolder; $RuntimeVLWinFolder; $ServerIconMacPath; $ServerIconWinPath : Text
	var $ServerIncludeIt; $ServerMacFolder; $ServerSelectionAllowed; $ServerWinFolder; $stringValue; $ToEmbedInClientMacFolder : Text
	var $ToEmbedInClientWinFolder; $UseStandardZipFormat; $Versioning : Text
	var $boolValue : Boolean
	var $i; $intValue; $j : Integer
	...
	$BuildApp.RuntimeVL:=New object:C1471(\
		"LastDataPathLookup"; "ByAppName")
	
	$BuildApp.SignApplication:=New object:C1471(\
