Class constructor($credentials : Object)
	...
	If ($credentials#Null:C1517)
		...
		If (This:C1470.identity.length#0)
			...
			var $identity : Object
			$identity:=This:C1470.identity.query("name == :1"; "Developer ID Application:@").pop()
			If ($identity#Null:C1517)
				This:C1470.signingIdentity:=$identity.name
			End if 
...
Function _staple($file : 4D:C1709.File)->$status : Object
	...
	LAUNCH EXTERNAL PROCESS:C811("xcrun stapler staple "+_tempo_escape_param($file.path); $stdIn; $stdOut; $stdErr; $pid)
...
Function _altool($params : Object)->$status : Object
	...
	Case of 
		...
		: (OB Instance of:C1731($params.file; 4D:C1709.File))
			$command:="xcrun altool --notarize-app --file "+_tempo_escape_param($params.file.path)
		Else 
			$command:=""
	End case 
	...
	If ($command#"")
		If (Value type:C1509(This:C1470.primaryBundleId)=Is text:K8:3)
			$command:=$command+" --primary-bundle-id "+_tempo_escape_param(This:C1470.primaryBundleId)
		End if 
		If (Value type:C1509(This:C1470.ascProvider)=Is text:K8:3)
			$command:=$command+" --asc-provider "+_tempo_escape_param(This:C1470.ascProvider)
		End if 
		If (Value type:C1509(This:C1470.username)=Is text:K8:3)
			$command:=$command+" --username "+_tempo_escape_param(This:C1470.username)
		End if 
		If (Value type:C1509(This:C1470.password)=Is text:K8:3)
			$command:=$command+" --password "+_tempo_escape_param(This:C1470.password)
		End if 
		
		If (This:C1470.ENVIRONMENT.DEVELOPER_DIR#Null:C1517)
			var $DEVELOPER_DIR : Text
...
Function _removeComponentPlugins($app : 4D:C1709.Folder; $statuses : Collection)->$this : cs:C1710.SignApp
	...
	var $file : 4D:C1709.File
	var $folder; $component; $plugins : 4D:C1709.Folder
...
Function _signMobile($app : 4D:C1709.Folder; $statuses : Collection)->$this : cs:C1710.SignApp
	...
	var $platforms; $extensions : Collection
	...
	For each ($file; $app.folder("Contents").folder("Resources").folder("Internal User Components").folder("4D Mobile App.4dbase").folder("Resources").folder("sdk").files(fk recursive:K87:7 | fk ignore invisible:K87:22).query("extension == :1"; ".zip"))
		var $temporaryFolder; $platformFolder; $folder : 4D:C1709.Folder
...
Function _deleteTempFiles($app : 4D:C1709.Folder)->$this : cs:C1710.SignApp
	...
	If (OB Instance of:C1731($app; 4D:C1709.Folder))
		...
		var $file : 4D:C1709.File
		For each ($file; $app.files(fk recursive:K87:7).query("extension == :1"; ".cstemp"))
			$file.delete()
		End for each 
...
Function pkgbuild($src : Object; $dst : Object)->$status : Object
	...
	If (Is macOS:C1572)
		...
		If (OB Instance of:C1731($src; 4D:C1709.File)) | (OB Instance of:C1731($src; 4D:C1709.Folder))
			...
			var $payloadFolder : 4D:C1709.Folder
			...
			var $command : Text
			$command:="cp -R "+\
				_tempo_escape_param($src.path)+" "+\
				_tempo_escape_param($payloadFolder.folder("payload").path)
			...
			$command:="pkgbuild --sign '"+\
				...
				_tempo_escape_param($installLocation)+\
				" --component-plist component.plist "+\
				_tempo_escape_param($dst.path)+" --identifier "+This:C1470.primaryBundleId
...
Function codesign($app : Object; $hardenedRuntime : Boolean; $options : Object)->$status : Object
	...
	If (OB Instance of:C1731($app; 4D:C1709.Folder))
		...
		var $infoPlistFile : 4D:C1709.File
	...
	If (Bool:C1537($options.remove))
		...
		$command:="codesign --remove-signature "+\
			_tempo_escape_param($app.name+$app.extension)
	Else 
		...
		If (Not:C34($hardenedRuntime))
			...
			$command:=$command+" --sign "+\
				_tempo_escape_param(This:C1470.signingIdentity)+" "+\
				_tempo_escape_param($app.name+$app.extension)
			
		Else 
			
			//create entitlements.plist
			var $entitlementsFolder : 4D:C1709.Folder
			...
			var $dom; $doctype; $dict; $value : Text
			...
			SET ENVIRONMENT VARIABLE:C812("_4D_OPTION_BLOCKING_EXTERNAL_PROCESS"; "TRUE")
			LAUNCH EXTERNAL PROCESS:C811("plutil -convert xml1 "+_tempo_escape_param($entitlementsFile.name+$entitlementsFile.extension); $stdIn; $stdOut; $stdErr; $pid)
			
			$command:=$command+" --options=runtime --entitlements "+\
				_tempo_escape_param($entitlementsFile.path)+" --sign "+\
				_tempo_escape_param(This:C1470.signingIdentity)+" "+\
				_tempo_escape_param($app.name+$app.extension)
	...
	If (OB Instance of:C1731($infoPlistFile; 4D:C1709.File))
		...
		If (OK=1)
			...
			If (OK=1)
				...
				SET ENVIRONMENT VARIABLE:C812("_4D_OPTION_BLOCKING_EXTERNAL_PROCESS"; "TRUE")
				LAUNCH EXTERNAL PROCESS:C811("plutil -convert xml1 "+_tempo_escape_param($infoPlistFile.name+$infoPlistFile.extension); $stdIn; $stdOut; $stdErr; $pid)
				...
				If (Not:C34(Bool:C1537($options.remove)))
					...
					$command:="codesign --verbose --sign "+\
						_tempo_escape_param(This:C1470.signingIdentity)+" "+\
						_tempo_escape_param($infoPlistFile.name+$infoPlistFile.extension)
...
Function getXcodePath()->$Xcode : Object
	...
	If (Is macOS:C1572)
		...
		SET ENVIRONMENT VARIABLE:C812("_4D_OPTION_BLOCKING_EXTERNAL_PROCESS"; "TRUE")
		LAUNCH EXTERNAL PROCESS:C811("mdfind "+_tempo_escape_param("kMDItemCFBundleIdentifier == 'com.apple.dt.Xcode'"); $stdIn; $stdOut; $stdErr; $pid)
...
Function listProviders()->$providers : Collection
	...
	If (Is macOS:C1572)
		...
		If ($Xcode.paths.length#0)
			var $iTMSTransporter : 4D:C1709.File
			...
			If (Is macOS:C1572)
				...
				SET ENVIRONMENT VARIABLE:C812("_4D_OPTION_BLOCKING_EXTERNAL_PROCESS"; "TRUE")
				LAUNCH EXTERNAL PROCESS:C811("iTMSTransporter -m provider -u "+_tempo_escape_param(This:C1470.username)+" -p "+_tempo_escape_param(This:C1470.password); $stdIn; $stdOut; $stdErr; $pid)
