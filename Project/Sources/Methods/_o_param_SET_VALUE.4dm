//%attributes = {"invisible":true}
  // ----------------------------------------------------
  // Method :  param_SET_VALUE
  // Created 21/05/10 by Vincent de Lachaux
  // ----------------------------------------------------
  // Description
  //
  // ----------------------------------------------------
  // Declarations
C_TEXT:C284($1)
C_TEXT:C284($2)
C_POINTER:C301($3)

C_LONGINT:C283($Lon_parameters;$Lon_x)
C_POINTER:C301($Ptr_value)
C_TEXT:C284($Dom_element;$Dom_root;$Txt_attributeName;$Txt_XPATH)

If (False:C215)
	C_TEXT:C284(_o_param_SET_VALUE ;$1)
	C_TEXT:C284(_o_param_SET_VALUE ;$2)
	C_POINTER:C301(_o_param_SET_VALUE ;$3)
End if 

  // ----------------------------------------------------
  // Initialisations
$Lon_parameters:=Count parameters:C259

If (Asserted:C1132($Lon_parameters>=3))
	
	$Dom_root:=$1
	$Txt_XPATH:=$2
	$Ptr_value:=$3
	
End if 

If (Position:C15("appMaker";$Txt_XPATH)#1)
	
	$Txt_XPATH:="appMaker/"+$Txt_XPATH
	
End if 

$Lon_x:=Position:C15("@";$Txt_XPATH;*)

If ($Lon_x>0)
	
	$Txt_attributeName:=Substring:C12($Txt_XPATH;$Lon_x+1)
	$Txt_XPATH:=Substring:C12($Txt_XPATH;1;$Lon_x-1)
	
End if 

  // ----------------------------------------------------
If (Asserted:C1132(Length:C16($Txt_attributeName)>0))
	
	$Dom_element:=DOM Find XML element:C864($Dom_root;$Txt_XPATH)
	
	If (OK=0)
		
		$Dom_element:=DOM Create XML element:C865($Dom_root;$Txt_XPATH)
		
	End if 
	
	If (Asserted:C1132(OK=1))
		
		If (Is nil pointer:C315($Ptr_value))
			
			DOM REMOVE XML ATTRIBUTE:C1084($Dom_element;$Txt_attributeName)
			
		Else 
			
			DOM SET XML ATTRIBUTE:C866($Dom_element;$Txt_attributeName;$Ptr_value->)
			
		End if 
	End if 
End if 